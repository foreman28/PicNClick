    ...
    <Layout.Footer className={styles.footer} style={{height: 'auto'}}>
      <Flex className={styles.footer_container} gap={64} justify={"space-between"} style={{width: '100%'}}>
        ...
        <Flex gap={48} className={styles.list}>
            ...
            <Flex vertical className={styles.footer__items}>
              <ul>
                {isLoadingLikes ?
                  ...
                  :
                  <>
                    {likesPosts && likesPosts?.posts.map(({id, ...post}: any) => (
                      <li key={id}>
                        <Link to={`${Paths.forum}/${post.url}`} className={styles.footer__item}>{post.title}</Link>
                      </li>
                    ))}
                  </>
                }
          ...
          <Flex vertical>
            <h5 className={styles.footer__title}>Обсуждаемые</h5>
            <Flex vertical className={styles.footer__items}>
              <ul>
                {isLoadingPosts ?
                  <></>
                  :
                  <>
                    {commentsPosts && commentsPosts?.posts.map(({id, ...post}: any) => (
                      <li key={id}>
                        <Link to={`${Paths.forum}/${post.url}`} className={styles.footer__item}>{post.title}</Link>
                      </li>
                    ))}
